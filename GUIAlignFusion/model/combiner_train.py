...
def load_pretrained_model(pretrained_path: str, clip_model: nn.Module, device: torch.device):
    """加载预训练模型"""
...
def save_model(name: str, cur_epoch: int, clip_model: nn.Module, combining_function: nn.Module,
    ...
    if is_combiner:
        model_path = models_path / f'combiner3_{name}.pt'
    else:
...
def clip_finetune_combiner(
    ...
    """第二阶段：在预训练模型基础上训练Combiner模块"""
    # === 初始化训练环境 ===
    training_start = datetime.now().strftime("%Y-%m-%d_%H:%M:%S")
    training_path = Path(base_path / f"combiner_training1/{clip_model_name}_{training_start}")
    training_path.mkdir(exist_ok=True, parents=True)

    print(f"Combiner训练开始时间: {training_start}")
    ...
    with open(training_path / "combiner_training_hyperparameters.json", 'w+') as file:
    ...
    # 加载预训练模型
    clip_model = load_pretrained_model(pretrained_path, clip_model, device)
    ...
    projection_dim = feature_dim // 2  # 投影到一半维度
    hidden_dim = feature_dim * 2  # 隐藏层维度加倍

    print(f"初始化Combiner模块:")
    print(f"  输入特征维度: {feature_dim}")
    print(f"  投影维度: {projection_dim}")
    print(f"  隐藏层维度: {hidden_dim}")

    combining_function = Combiner(feature_dim, projection_dim, hidden_dim).to(device)

    # 打印Combiner参数量
    combiner_params = sum(p.numel() for p in combining_function.parameters() if p.requires_grad)
    print(f"Combiner可训练参数: {combiner_params:,}")
    ...
    # 智能早停参数
    min_improvement = 0.001  # 初始最小改进阈值 (0.1%)
    min_improvement_decay = 0.95  # 每10轮衰减改进阈值
    min_improvement_limit = 0.0001  # 最小改进阈值下限
    ...
    for epoch in range(num_epochs):
        ...
        # 设置模型模式
        clip_model.eval()  # CLIP始终冻结
        combining_function.train()  # Combiner可训练
        ...
        with experiment.train():
            ...
            for idx, batch in enumerate(train_bar):
                ...
                with torch.cuda.amp.autocast():
                    ...
                    # 使用Combiner进行特征融合
                    logits = combining_function(reference_features, caption_features, target_features)
                ...
                torch.nn.utils.clip_grad_norm_(combining_function.parameters(), max_norm=1.0)
            ...
            with torch.no_grad():
                ...
                with experiment.validate():
                    ...
                    recall_at10, recall_at50, mrr, precision_at10, precision_at50 = compute_gui_val_metrics(
                        ...
                        clip_model,
                        combining_function,  # 使用新的Combiner
                        index_features,
                        index_names
                    )
                ...
                if save_training:
                    if save_best:
                        ...
                        # 确保阈值不会低于下限
                        current_min_improvement = max(current_min_improvement, min_improvement_limit)
                        ...
                        if smoothed_recall > best_avg_recall + current_min_improvement:
                            ...
                            best_epoch = epoch
                            model_save_name = f'combiner_best_epoch{epoch + 1}_recall{smoothed_recall:.4f}'
                            best_model_path = save_model(model_save_name, epoch,
                                                         clip_model, combining_function,
                                                         training_path, is_combiner=True)
                            early_stop_count = 0
                        ...
                        save_model(f'combiner_{epoch}', epoch, clip_model, combining_function,
                                   training_path, is_combiner=True)
    ...
    if save_training:
        if save_best and best_model_path:
            # 复制最佳Combiner为最终模型
            final_model_path = save_model('combiner_final', best_epoch,
                                          clip_model, combining_function,
                                          training_path, is_combiner=True)
            print(f"最终Combiner基于轮次 {best_epoch + 1} 的最佳模型")
        else:
            print("保存当前Combiner为最终模型")
            save_model('combiner_final', num_epochs - 1, clip_model, combining_function,
                       training_path, is_combiner=True)

    # 训练总结报告
    total_duration = datetime.now() - datetime.strptime(training_start, "%Y-%m-%d_%H:%M:%S")
    print("\n" + "=" * 60)
    print(f"Combiner训练总结 - 总轮数: {epoch + 1}/{num_epochs}")
...
if __name__ == '__main__':
    # 参数解析与实验初始化
    parser = ArgumentParser(description="CLIP Combiner训练脚本")
    ...
    parser.add_argument("--learning-rate", default=5e-5, type=float, help="学习率")
    ...
    if args.api_key and args.workspace:
        ...
        experiment = Experiment(
            ...
            project_name=f"GUI_Combiner_Training",
        ...
        if args.experiment_name:
            ...
            experiment.set_name(f"Combiner_{args.clip_model_name}_{datetime.now().strftime('%Y%m%d_%H%M')}")
    else:
    ...
    # 启动训练
    training_path = clip_finetune_combiner(**training_hyper_params)
